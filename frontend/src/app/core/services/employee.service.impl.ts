...
export class EmployeeServiceImpl extends EmployeeService {
  ...
  override getEmployeeById(id: number): Observable<Employee> {
    return this.http.get<Employee>(`${this.baseUrl}/${id}`);
  }

  override createEmployee(request: EmployeeCreateRequest): Observable<Employee> {
    return this.http.post<Employee>(this.baseUrl, request);
  }

  override updateEmployee(id: number, request: EmployeeUpdateRequest): Observable<Employee> {
  ...
  override getSalaryHistory(id: number): Observable<SalaryRecord[]> {
